layui.use(['layer', 'form', 'table'], function () {
    ...
    t = {
        ...
        cols: [[
            {type: 'checkbox'},
            {field: 'xueshengId', title: '学号', width: '15%'},
            {field: 'personalName', title: '姓名', width: '20%'},
            {field: 'remark', title: '财物描述', width: '40%'},
    ...
    table.on('tool(userList)', function (obj) {
        ...
        if (obj.event === 'edit') {
            var editIndex = layer.open({
                title: "编辑用户",
                type: 2,
                content: "/asaCaichan/edit?id=" + data.id,
        ...
        if (obj.event === "del") {
            layer.confirm("你确定要删除该用户么？", {btn: ['是的,我确定', '我再想想']},
                function () {
                    $.post("/asaCaichan/delete", {"id": data.id}, function (res) {
                        if (res.success) {
