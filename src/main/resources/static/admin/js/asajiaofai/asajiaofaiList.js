layui.use(['layer', 'form', 'table'], function () {
    ...
    t = {
        ...
        cols: [[
            {type: 'checkbox'},
            {field: 'sushehao', title: '宿舍号', width: '10%'},
            {field: 'jiaofaileixing', title: '缴费类型', width: '10%'},
            {field: 'jiaofaijine', title: '缴费金额', width: '10%'},
            {
                field: 'jiaofaishijian',
                title: '缴费时间',
                width: '14%',
                templet: '<span>{{ layui.laytpl.toDateString(d.jiaofaishijian) }}</span>'
            }, //单元格内容水平居中
            {field: 'jiaofairen', title: '缴费人', width: '12%'},
            {field: 'chuliren', title: '处理人', width: '12%'},
            {field: 'beizhu', title: '备注', width: '12%'},
    ...
    table.on('tool(userList)', function (obj) {
        ...
        if (obj.event === 'edit') {
            var editIndex = layer.open({
                title: "编辑用户",
                type: 2,
                content: "/asaJiaofai/edit?id=" + data.id,
        ...
        if (obj.event === "del") {
            layer.confirm("你确定要删除该用户么？", {btn: ['是的,我确定', '我再想想']},
                function () {
                    $.post("/asaJiaofai/delete", {"id": data.id}, function (res) {
                        if (res.success) {
